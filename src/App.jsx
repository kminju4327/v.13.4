...
같은 JSON 형식으로 재작성된 섹션만 반환하세요.`;
  ...
  async function runPipeline(forceDetailRegeneration = false) {
    ...
    // 설계가 있으면 상세페이지 생성 실행
    // 생성 중에는 현재 AI 설계/템플릿 화면을 유지해 제품 입력 화면이 잠깐 노출되지 않도록 합니다.
...
<body>
  ...
  return (
    <div style={{ fontFamily: '"Pretendard", "Apple SD Gothic Neo", "Malgun Gothic", sans-serif', background: "#F8F5EF", minHeight: "100%", color: "#26231F" }}>
      <style>{`
        .redesigned-input .input-page-head, .redesigned-input .input-progress, .redesigned-input .input-workspace-grid { width: 100%; max-width: 1240px; margin-left: auto; margin-right: auto; }
        .redesigned-input .input-page-head { padding-bottom: 14px; border-bottom: 1px solid #E9E1D6; }
        .redesigned-input .input-progress { margin-top: 14px; margin-bottom: 18px; }
        .redesigned-input .input-workspace-grid { margin-top: 0; }
        .redesigned-input .input-form-column { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 18px; align-items: start; min-width: 0; }
        .redesigned-input .input-section-card { display: flex; flex-direction: column; gap: 15px; padding: 20px 22px 22px; background: rgba(255,255,255,0.96); border: 1px solid #E6DED3; border-radius: 14px; box-shadow: 0 8px 24px rgba(70,52,32,0.035); min-width: 0; }
        .redesigned-input .input-section-title { display: flex; align-items: center; gap: 10px; min-height: 32px; padding-bottom: 12px; border-bottom: 1px solid #EEE7DD; font-size: 14px; font-weight: 850; color: #342A21; }
        .redesigned-input .input-section-number { width: 22px; height: 22px; flex: 0 0 22px; border-radius: 50%; border: 1.5px solid #B87A2C; color: #A6681F; display: inline-flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 900; background: #FFFDF9; }
        .redesigned-input .category-select-grid { display: grid !important; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 10px !important; }
        .redesigned-input .category-select-grid > div { grid-column: 1 / -1; }
        .redesigned-input .input-section-card:nth-child(3) { grid-column: 1 / -1; display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 15px 22px; align-items: start; }
        .redesigned-input .input-section-card:nth-child(3) > :first-child { grid-column: 1 / -1; }
        .redesigned-input .input-section-card:nth-child(3) > :nth-child(6),
        .redesigned-input .input-section-card:nth-child(3) > :nth-child(n+7) { grid-column: 1 / -1; }
        .redesigned-input .input-generate-row { grid-column: 1 / -1; display: flex; justify-content: center; padding: 4px 0 10px; }
        .redesigned-input .input-generate-row button { min-height: 46px; }
        @media (max-width: 1180px) {
          .redesigned-input .input-section-card:nth-child(3) { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        }
        @media (max-width: 980px) {
          .redesigned-input .input-form-column { grid-template-columns: 1fr; }
          .redesigned-input .input-section-card:nth-child(3) { grid-column: auto; }
        }
        @media (max-width: 720px) {
          .product-input-shell.redesigned-input { padding: 24px 18px 40px !important; }
          .redesigned-input .input-section-card { padding: 18px; border-radius: 14px; }
          .redesigned-input .input-section-card:nth-child(3) { grid-template-columns: 1fr; }
          .redesigned-input .input-section-card:nth-child(3) > :nth-child(n) { grid-column: auto; }
          .redesigned-input .category-select-grid { grid-template-columns: 1fr; }
          .redesigned-input .category-select-grid > div { grid-column: auto; }
        }
      `}</style>
      <AppHeader 
        currentProjectName={projectName || product?.name || "새 프로젝트"}
      ...
      <div className="app-grid" style={{
        display: "grid",
        gridTemplateColumns: isInitialInputLayout
          ? "168px minmax(0, 1fr)"
        ...
        {viewMode === "main" ? (
          ...
          <div className="input-progress" style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {["입력", "생성 중", "완료"].map((s, i) => (
          ...
          {draft && (
            <div style={{ maxWidth: 940, margin: "0 auto", fontFamily: bodyFamily }}>
              <PreviewSection
                ...
                accent={themeColor}
                onEdit={() => setEditingSection({ id: "hero", section: { title: displayDraft.hero_headline || "", body: displayDraft.hero_subcopy || "", items: [] } })}
                onHistory={() => setHistorySection({ id: "hero", label: "메인 섹션" })}
              >
                <div
                  style={{
                    position: "relative",
              ...
              {displayDraft.sections?.map((s, i) => {
                ...
                return (
                  <PreviewSection
                    ...
                    accent={themeColor}
                    onEdit={() => setEditingSection({ id: s.id || s.type || i, section: s })}
                    onHistory={() => setHistorySection({ id: s.id || s.type || i, label: s.title || sectionLabel(s.type) || `섹션 ${i + 1}` })}
                  >
                    <div
                      style={{
                        position: "relative",
...
function PreviewSection({ idx, onRegen, onEdit, onHistory, loading, accent, children }) {
  return (
    <div style={{ position: "relative", marginBottom: 18 }}>
      {children}
      <div style={{ position: "absolute", top: 12, right: 12, zIndex: 3, display: "flex", alignItems: "center", gap: 2, padding: 3, borderRadius: 9, border: "1px solid rgba(31,42,36,0.10)", background: "rgba(255,255,255,0.94)", boxShadow: "0 4px 14px rgba(31,42,36,0.08)" }}>
        {onEdit && (
          <button
            type="button"
            onClick={onEdit}
            style={{ border: "none", background: "transparent", color: "#5A4A47", padding: "6px 9px", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer" }}
          >
            편집
          </button>
        )}
        {onEdit && onHistory && <span aria-hidden="true" style={{ width: 1, height: 14, background: "#E8E1D7" }} />}
        {onHistory && (
          <button
            type="button"
            onClick={onHistory}
